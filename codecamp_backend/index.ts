...
const resolvers = {
  ...
  Mutation: {
    createBoard: async (parent: any, args: any, contents: any, info: any) => {
      await Board.insert({
    ...
    //     await Board.update({number: 3}, {isDeleted: true}); // 3번 게시글 삭제했다 치자! (소프트삭제) => isDeleted가 초기값인 false이면? 삭제 안된거, true 이면? 삭제 된거
    //     await Board.update({number: 3}, {deletedAt: new Date() }) // 3번 게시글 삭제했다 치자! (소프트삭제) => deletedAt이 초기값인 Null이면? 삭제 안된거, new Date() 들어가 있으면 삭제 된거
    // },
  },
};
